...
export function PostEditor({
  ...
  const [donguriState, setDonguriState] = useState<DonguriState>({
    status: 'none',
    message: '',
    loggedIn: false,
  });
              ...
              {tripHelpOpen && (
                <div className="absolute left-0 top-full z-50 mt-1 w-64 rounded border border-[var(--color-border-primary)] bg-[var(--color-bg-secondary)] p-3 text-xs shadow-lg">
                  ...
                  <p className="mb-1 text-[var(--color-text-secondary)]">
                    名前欄に{' '}
                    <code className="rounded bg-[var(--color-bg-tertiary)] px-1">
                      名前#パスワード
                    </code>{' '}
                    と入力すると、 パスワードからトリップ（固有ID）が生成されます。
                  </p>
                  <ul className="ml-3 list-disc space-y-0.5 text-[var(--color-text-muted)]">
                    <li>
                      <code>#</code> の後に好きな文字列を入力
                    </li>
                    <li>8文字以下は10桁トリップ</li>
                    <li>
                      12文字以上で <code>##</code> を使うと新方式トリップ
                    </li>
            ...
            <div className="relative">
              <button
                ...
                className={`flex items-center gap-1 rounded px-1 py-0.5 text-xs hover:bg-[var(--color-bg-hover)] ${
                  donguriState.status === 'active'
                    ? 'text-green-400'
                    : donguriState.status === 'broken'
                      ? 'text-[var(--color-error)]'
                      : donguriState.status === 'consumed'
              ...
              {donguriPopupOpen && (
                <>
                  ...
                  <div
                    ...
                    style={{
                      left: donguriPopupPos.x,
                      bottom: window.innerHeight - donguriPopupPos.y + 4,
                    }}
                  >
                    <h4 className="mb-1 font-bold text-[var(--color-text-primary)]">
                      どんぐりステータス
                    ...
                    <table className="w-full text-[var(--color-text-secondary)]">
                      <tbody>
                        ...
                        {donguriState.loggedIn === true &&
                          donguriState.cannonStats !== undefined && (
                            <tr>
                        ...
                        {donguriState.loggedIn === true &&
                          donguriState.fightStats !== undefined && (
                            <tr>
                    ...
                    {donguriState.loggedIn !== true && (
                      <div className="mt-2 space-y-1 rounded border border-[var(--color-border-secondary)] bg-[var(--color-bg-primary)] p-2">
                        <p className="text-[10px] text-[var(--color-text-muted)]">
                          どんぐりにログインして詳細ステータスを取得
                        </p>
                        <input
                          type="email"
                          value={donguriMail}
                          onChange={(e) => {
                            setDonguriMail(e.target.value);
                          }}
                          placeholder="メールアドレス"
                        ...
                        <input
                          ...
                          onChange={(e) => {
                            setDonguriPassword(e.target.value);
                          }}
                          placeholder="パスワード"
                          className="w-full rounded border border-[var(--color-border-primary)] bg-[var(--color-bg-secondary)] px-2 py-1 text-xs text-[var(--color-text-primary)] placeholder:text-[var(--color-text-muted)] focus:border-[var(--color-accent)] focus:outline-none"
                        />
            ...
            <label className="ml-auto flex cursor-pointer items-center gap-1 text-xs text-[var(--color-text-muted)]">
              <input
                ...
                onChange={(e) => {
                  ...
                  try {
                    localStorage.setItem(AUTO_CLOSE_KEY, String(e.target.checked));
                  } catch {
                    /* ignore */
                  }
                }}
