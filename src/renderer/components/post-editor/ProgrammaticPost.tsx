...
import { mdiPlay, mdiStop, mdiPlus, mdiDelete, mdiClose, mdiFileUpload, mdiDownload } from '@mdi/js';
...
export function ProgrammaticPost({ boardUrl, threadId, onClose }: ProgrammaticPostProps): React.JSX.Element {
  ...
  const handleStartManual = useCallback(async () => {
    ...
    name, mail, message, doPost, addLog, waitMs,
  ]);

  /** Execute a fully-parsed DSL script */
  const executeDslScript = useCallback(async (script: DslScript): Promise<void> => {
    ...
    const flatPosts: Array<{ name: string; mail: string; message: string; intervalSec: number | undefined; isLast: boolean }> = [];
  ...
  const handleStartDsl = useCallback(async () => {
    ...
    if (dslSource.trim().length === 0) {
      setDslParseError('スクリプトが空です。ファイルを読み込むか、直接入力してください。');
      return;
    }
    ...
    if (!result.ok) {
      setDslParseError(
        result.errors.map((e) => (e.line > 0 ? `行${String(e.line)}: ${e.message}` : e.message)).join('\n'),
      );
      return;
    }
      ...
      <div className="flex-1 overflow-auto px-3 pb-3 pt-2">
        {mode === 'manual' ? (
          ...
          <>
            ...
            <textarea
              value={dslSource}
              onChange={(e) => { setDslSource(e.target.value); setDslParseError(''); }}
              placeholder={'# ファイルを開くかここに直接入力\nPOST\nNAME\nMAIL sage\nMESSAGE テスト投稿\nEND'}
              spellCheck={false}
              rows={6}
              className="mb-1 w-full resize-y rounded border border-[var(--color-border-primary)] bg-[var(--color-bg-primary)] px-2 py-1 font-mono text-xs leading-relaxed text-[var(--color-text-primary)] placeholder:text-[var(--color-text-muted)] focus:outline-none"
            />
            ...
            {dslParseError.length > 0 && (
              <div className="mb-1 rounded border border-[var(--color-error)] bg-[var(--color-bg-primary)] p-1">
                {dslParseError.split('\n').map((line, i) => (
                  <p key={i} className="text-[10px] text-[var(--color-error)]">{line}</p>
                ))}
              </div>
            )}
          </>
