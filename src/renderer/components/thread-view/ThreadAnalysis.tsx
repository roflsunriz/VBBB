  ...
  return (
    <div className="space-y-0.5">
      {entries.map((e) => {
        ...
        const snippet =
          firstRes !== undefined ? truncate(toPlainText(firstRes.body), SNIPPET_LENGTH) : '';
        return (
          <button
            key={e.key}
            type="button"
            onClick={() => {
              onClickEntry(e.resNumbers);
            }}
      ...
      {entries.length === 0 && <p className="text-xs text-[var(--color-text-muted)]">データなし</p>}
    </div>
  );
}
...
export function ThreadAnalysis({
  ...
  onClose,
  onScrollToRes,
}: ThreadAnalysisProps): React.JSX.Element {
  const analysis: ThreadAnalysisResult = useMemo(() => analyzeThread(responses), [responses]);

  /** Map from res number → Res for fast lookup */
  const resMap = useMemo(() => new Map(responses.map((r) => [r.number, r])), [responses]);
      ...
      >
        ...
        <div className="flex-1 overflow-y-auto">
          ...
          <CollapsibleSection title="コテハン一覧" count={analysis.kotehanRanking.length}>
            <CountList
              entries={analysis.kotehanRanking}
              resMap={resMap}
              onClickEntry={handleClickEntry}
            />
          </CollapsibleSection>
          ...
          <CollapsibleSection title="必死レス（長文）" count={analysis.longPosts.length}>
            <div className="space-y-0.5">
              {analysis.longPosts.map((lp) => {
                ...
                const snippet =
                  res !== undefined ? truncate(toPlainText(res.body), SNIPPET_LENGTH) : '';
                return (
                  <button
                    key={lp.resNumber}
                    type="button"
                    onClick={() => {
                      onScrollToRes(lp.resNumber);
                    }}
                    className="flex w-full flex-col rounded px-1 py-1 text-left text-xs text-[var(--color-text-secondary)] hover:bg-[var(--color-bg-hover)]"
                  >
                    <div className="flex w-full items-center gap-2">
          ...
          <CollapsibleSection
            title="必死レス（ワッチョイ別）"
            count={analysis.watchoiRanking.length}
          >
            <CountList
              entries={analysis.watchoiRanking}
              resMap={resMap}
              onClickEntry={handleClickEntry}
            />
          </CollapsibleSection>
